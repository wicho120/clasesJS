...
export const getAllOficceAndCodeCity = async()=>{
    let res = await fetch("http://176.16.101.146:5743/offices")
...
export const getAllOficceCityAndMovil = async()=>{
    let res = await fetch("http://176.16.101.146:5743/offices?country=España")
    let data = await res.json();
    let dataUpdate = data.map(val =>{
        return {
            code_office: val.code_office,
...
export const getAllInfoOfficeByCode_Office = async(code) => {
    let res = await fetch(`http://176.16.101.146:5743/offices?code_office=${code}`)
    let data = await res.json();
